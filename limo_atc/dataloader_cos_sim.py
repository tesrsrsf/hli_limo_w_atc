...
class DataManager:
    ...
    def __init__(self, datas, batch_size, max_len, human_label, id2label, word_pad_idx=0, label_pad_idx=-1, at_feature_lookup=None, global_at_feature_lookup=None):
        set_seed(0)
        ...
        self.at_feature_lookup = at_feature_lookup      # local at_feature
        self.global_at_feature_lookup = global_at_feature_lookup # global at_feature
    ...
    def get_global_atf_mat(self, problem_id, raw_text_all):
        """
        从 great_data_doc sidecar 取出 doc-level ATF 矩阵:
        shape: [n_lines, 128]（每一行都一样）
        """
        if self.global_at_feature_lookup is None:
            return None

        lookup = self.global_at_feature_lookup

        # AtcSidecar 的情况
        if isinstance(lookup, AtcSidecar):
            arr = lookup.get(problem_id=problem_id, full_text=raw_text_all)
            return arr

        # 兼容 dict 风格
        sha = hashlib.sha1(raw_text_all.encode('utf-8')).hexdigest()
        doc_key = f"{str(problem_id)}::{sha}"
        arr = (lookup.get(doc_key) or lookup.get((problem_id, sha)) or lookup.get(problem_id))
        return arr
    ...
    def initialize_dataset(self, total_samples):
        ...
        for item in tqdm(total_samples):
            ...
            # NEW: 取到了 sidecar：乘以作为 weight 的 global atf 的 cosine similarity
            local_mat = None
            global_mat = self.get_global_atf_mat(problem_id, raw_text_all)  # [n_lines, 128]
            if atfeatures_list is not None:
                local_mat = np.array(atfeatures, dtype=np.float32)  # [n_lines, 128]
                global_mat = np.array(global_mat, dtype=np.float32)

            T = min_len - max_begin_idx

            if local_mat is not None: 
                local_slice = local_mat
                if global_mat is not None and global_mat.size > 0:
                    global_atf = global_mat[0]  # [128,], all lines are same
                    global_atf_norm = np.linalg.norm(global_atf) + 1e-8

                    a_norm = np.linalg.norm(local_slice, axis=1, keepdims=True) + 1e-8  # [T,]
                    dotres = np.dot(local_slice, global_atf)  # [T,]
                    cosine_sim = (dotres / (a_norm.squeeze(-1) * global_atf_norm)).reshape(-1, 1)  # [T, 1]

                    cosine_sim = np.clip(cosine_sim, -1.0, 1.0)
                    weight = (cosine_sim + 1.0) / 2.0  # [T, 1], scale to [0,1], 0: totally different, 1: same
                    #weight = cosine_sim  
                    local_slice = local_slice * weight  # weighted local atf

                if local_slice.shape[0] < T: 
                    pad = np.zeros((T - local_slice.shape[0], 128), dtype=np.float32)
                    local_slice = np.vstack([local_slice, pad])
                elif local_slice.shape[0] > T: 
                    local_slice = local_slice[:T]

                atfeatures = local_slice.tolist()
...
class DataManagerTest:
    ...
    def __init__(self, datas, batch_size, max_len, human_label, id2label, word_pad_idx=0, label_pad_idx=-1, at_feature_lookup=None, global_at_feature_lookup=None):
        ...
        self.global_at_feature_lookup = global_at_feature_lookup  # <-- NEW
    ...
    def _get_global_atf_mat(self, problem_id, raw_text_all):
        """
        从 great_data_doc sidecar 取出 doc-level ATF 矩阵:
        shape: [n_lines, 128]（每一行都一样）
        """
    ...
    def initialize_dataset(self, total_samples):
        ...
        for item in tqdm(total_samples):
            ...
            if atfeatures_list is None:
                ...
                atfeatures = [[0.0]*128 for _ in range(total_lines)]
            else:
                atfeatures = atfeatures_list[max_begin_idx:min_len]
            ...
            global_mat = self.get_global_atf_mat(problem_id, raw_text_all)  # [n_lines, 128]
            if atfeatures_list is not None:
                local_mat = np.array(atfeatures_list, dtype=np.float32)  # [n_lines, 128]
                global_mat = np.array(global_mat, dtype=np.float32)

            T = min_len - max_begin_idx
            
            if local_mat is not None: 
                local_slice = local_mat[max_begin_idx:min_len]  # [T, 128]
                if global_mat is not None and global_mat.size > 0:
                    global_atf = global_mat[0]  # [128,], all lines are same
                    global_atf_norm = np.linalg.norm(global_atf) + 1e-8
