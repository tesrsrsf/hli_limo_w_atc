...
class MultiModalConcatLineFocalBMESBinaryClassifier(nn.Module):
    def __init__(self, id2labels, seq_len, intermediate_size = 512, num_layers=2, dropout_rate=0.1, alpha=0.5):
        ...
        '''
        # NEW: AT 特征编码器（输入维度 256）
        self.at_feature_encoder = TransformerClassifier(input_dim=256, hidden_dim=256, output_dim=1, n_heads=4, n_layers=2, lr=0.001, max_seq_len=1024)
    ...
    def forward(self, x, labels, ccfeature, atfeatures=None):
        ...
        if atfeatures is None:
            atfeatures = torch.zeros(outputs.size(0), T, 256, device=outputs.device, dtype=outputs.dtype)  # NEW: 256维全零张量
        else:
