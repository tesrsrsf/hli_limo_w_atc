...
# ==== ATC sidecar loader: read line-level 768D features by doc_key ====
class AtcSidecar:
    """
    ...
    def get(self, problem_id=None, full_text=None, doc_key=None):
        """
        返回: np.ndarray [num_lines, 768] 或 None
        ...
        if arr.ndim != 2 or arr.shape[1] != 768:
            raise ValueError(f"ATC sidecar dim mismatch: got {arr.shape}, expect [*,768], path={path}")
        ...
        for item in tqdm(total_samples):
            ...
            if atfeatures_list is not None:
                ...
                if len(atfeatures) < (min_len - max_begin_idx):
                    ...
                    atfeatures = atfeatures + [[0.0] * 768 for _ in range(need)]
                if len(atfeatures) > 0 and len(atfeatures[0]) != 768:
                    raise ValueError(f"AT feature dim mismatch: got {len(atfeatures[0])}, expect 768")
            else:
                atfeatures = [[0.0] * 768 for _ in range(min_len - max_begin_idx)]
    ...
    def initialize_dataset(self, total_samples):
        ...
        for item in tqdm(total_samples):
            ...
            if atfeatures_list is None:
                ...
                atfeatures_list = [[0.0]*768 for _ in range(total_lines)]
